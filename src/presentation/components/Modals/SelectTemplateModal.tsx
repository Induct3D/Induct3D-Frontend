...
export default function SelectTemplateModal({
    ...
    const handlePrev = (id: string) => {
        ...
        setActiveSlide((prev) => ({
            ...prev,
            [id]:
                ((prev[id] ?? 0) - 1 + template.images.length) %
                ...
                <Dialog.Panel className="w-full max-w-5xl rounded-xl bg-white shadow-lg p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[60vh] overflow-y-auto pr-2">
                        {isLoading ? (
                            ...
                            <p className="text-center col-span-3 text-gray-500">
                                Aún no tienes plantillas disponibles.
                            </p>
                        ) : (
                            templates.map((template) => (
                                <div
                                    ...
                                    className={`border rounded-xl p-4 cursor-pointer transition duration-200 hover:shadow-md ${
                                        selectedId === template.id ? "ring-2 ring-[#A71C20]" : ""
                                    }`}
                                >
                    ...
                    <div className="flex justify-end mt-6">
                        <button
                            ...
                            disabled={!selectedId}
                            className={`px-6 py-2 rounded-md text-white transition duration-150 ${
                                selectedId
                                    ? "bg-[#A71C20] hover:opacity-90 cursor-pointer"
